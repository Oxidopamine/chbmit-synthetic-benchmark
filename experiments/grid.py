...
subset (Rule 4) and reused across detectors. ``synthetic_aug`` cells iterate over
generators; skipped (below-threshold) generators are recorded, never silently
dropped. Per-generator quality checks run alongside (Section 20).
"""
...
def run_grid(
    ...
    needs_gen = "synthetic_aug" in grid.conditions and grid.generators
...
def _skipped_record(fold, seed, frac, detector, gen, reason):
    return {
        "spec": {"fold": fold, "seed": seed, "scarcity_fraction": frac,
                 "detector": detector, "condition": "synthetic_aug", "generator": gen,
